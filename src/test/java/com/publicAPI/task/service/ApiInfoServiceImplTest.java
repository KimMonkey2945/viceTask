...
public class ApiInfoServiceImplTest {
    ...
    @Autowired
    private ApiInfoRepository apiInfoRepository2;

    @Autowired
    private ApiInfoServiceImpl apiInfoServiceImpl2;

    @Test
    @DisplayName("openAPI searchByKeyword test")
    public void testSearchByKeyword(){
        String keyword = "우륵기념탑";
        List<ApiInfo> result = apiInfoServiceImpl2.searchByKeyword(keyword);
        Assertions.assertTrue(!result.isEmpty(), "empty");
    ...
    public void testSearchByMonthAndLocation(){
        ...
        List<ApiInfo> result = apiInfoServiceImpl2.searchByMonthAndLocation(month, location);
        Assertions.assertTrue(!result.isEmpty(), "empty");
